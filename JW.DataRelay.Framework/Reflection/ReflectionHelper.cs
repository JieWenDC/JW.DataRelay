...
namespace JW.DataRelay.Framework
{
    public partial class ReflectionHelper
    {
        ...
        /// 条件反射程序集缓存
        /// </summary>
        private static Dictionary<string, Assembly> CACHE_ASSEMBLY = new Dictionary<string, Assembly>();

        /// <summary>
        /// 方法参数缓存
        /// </summary>
        private static Dictionary<MethodInfo, ParameterInfo[]> CACHE_METHOD_PARAM = new Dictionary<MethodInfo, ParameterInfo[]>();

        /// <summary>
        /// 缓存方法信息
        /// </summary>
        private static Dictionary<string, MethodInfo> CACHE_METHOD = new Dictionary<string, MethodInfo>();
        ...
        {
            var assembly = GetAssembly(assemblyString);
            assembly.CheckNull($"命名空间:{assemblyString}不存在");
            var type = assembly.GetType(typeName);
            type.CheckNull($"类型({typeName})不存在与命名空间({assemblyString})下");
