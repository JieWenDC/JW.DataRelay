...
namespace System
{
    ...
    public static class ConvertExtend
    {
        ...
        /// <returns></returns>
        public static T ToNumber<T>(this object _this, T def = default(T))
        {
            ...
            else
            {
                ...
                return _this.ToString().ToNumber<T>();
            }
        }
        ...
        /// <param name="def">转换失败的默认值</param>
        /// <returns></returns>
        public static List<string> ToListData(this string _this, string separator = ",", List<string> def = null)
        {
            if (string.IsNullOrEmpty(_this))
            {
                return def;
            }
            var result = _this.Split(new string[] { separator }, StringSplitOptions.RemoveEmptyEntries).ToList();
            return result;
        }
        ...
        /// <param name="_this"></param>
        /// <returns></returns>
        public static string ToZH(this DateTime _this, string format = "MM/dd HH:mm")
        {
            if (_this.AddMinutes(3) > DateTime.Now)
            {
                return "刚刚";
            }
            if (_this.AddMinutes(59) > DateTime.Now)
            {
                return string.Format("{0}分钟前", (DateTime.Now - _this).Minutes);
            }
            if (_this.AddHours(23) > DateTime.Now)
            {
                return string.Format("{0}小时前", (DateTime.Now - _this).Hours);
            }
